...
class PostsDateReschedule{
	...
	public function hooks(){
		...
		load_plugin_textdomain( 'pdr', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
	}

	public function register_styles_scripts(){
	    if ( isset( $_GET['page'] ) && $_GET['page'] == 'posts-date-reschedule-settings' ){
            //Styles
            wp_enqueue_style( 'pdr-styles', PDR_URI . '/assets/styles/main.css', '', '1.0.1', null );

            //Scrpts
            wp_register_script( 'pdr-scripts', PDR_URI . '/assets/scripts/main.js', 'jquery', '1.0.1', false );

            $translation_array = array(
                'developed' => __( 'Developed by:', 'pdr' ),
            );

            wp_localize_script( 'pdr-scripts', 'translate', $translation_array );
            wp_enqueue_script( 'pdr-scripts' );
        }
	}
			...
			if ( isset($posts) ) {
				...
				if ( $pdt_function == 1 ) {
					...
					if ( isset($date) && isset($time) && $date != null && $time != null ) {
						...
						$pdr_messages[] = array(
							'type' => 'error',
							'message' => __( 'Please choose date & time', 'pdr' ),
						);
					}
				}
				...
				if ( $pdt_function == 2 || $pdt_function == 3 ) {
					if ( isset($_POST['pdt_function_days_plus']) && $_POST['pdt_function_days_plus'] != null || isset($_POST['pdt_function_days_minus']) && $_POST['pdt_function_days_minus'] != null ) {
						...
						$pdr_messages[] = array(
							'type' => 'error',
							'message' => __( 'Please choose days', 'pdr' ),
						);
					}
				}
				...
				if ( wp_update_post($post_info) ) {
					$pdr_messages[] = array(
						'type' => 'success',
						'message' => __( 'Updated the posts successfully', 'pdr' ),
					);
				}
				...
				$pdr_messages[] = array(
					'type' => 'error',
					'message' => __( 'Please choose posts', 'pdr' ),
				);
			}
		}
